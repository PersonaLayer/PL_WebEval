...
export class Tab {
  ...
  async title(): Promise<string> {
    // Try native document.title first
    const raw = await callOnPageNoTrace(this.page, page => page.title());
    const trimmed = (raw || '').trim();
    if (trimmed) return trimmed;

    // Fallbacks: DOM <title>, prominent headings, then hostname
    try {
      const fallback = await callOnPageNoTrace(this.page, page => page.evaluate(() => {
        const getHeading = () => {
          const selectors = ['h1', '[role="heading"][aria-level="1"]', 'h2', '[role="heading"]'];
        ...
        const titleText = (titleEl?.textContent || document.title || '').trim();
        const heading = getHeading();
        return titleText || heading || '';
      }));
      if ((fallback || '').trim()) return (fallback as string).trim();
    } catch {
